...
export class WNPRedux {
  ...
  constructor() {
    ...
    //   this.mediaInfo.durationSeconds = Math.round(
    //     parseInt(meta.duration) / 1000
    //   );
    //   this.mediaInfo.state = data.is_paused
    //     ? StateMode.PAUSED
    //     : StateMode.PLAYING;
    //   this.mediaInfo.repeatMode = data.options.repeating_track
    //     ? RepeatMode.ONE
    ...
    //     ([added]: any) => (this.mediaInfo.rating = added ? 5 : 0)
    //   );

    //   const cover = meta.image_xlarge_url;
    //   if (cover?.indexOf("localfile") === -1)
    //     this.mediaInfo.coverUrl =
  ...
  public reloadSockets() {
    ...
    for (const adapter of BuiltInAdapters) {
      if (this.settings.enabledBuiltInAdapters.includes(adapter.name))
        this.sockets.set(
          adapter.port,
          new WNPReduxWebSocket(adapter, this.executeEvent.bind(this))
        );
    }
  ...
  public updateAll() {
    if (!Spicetify.Player.data?.track?.metadata) return;
    const meta = Spicetify.Player.data.track;
    this.mediaInfo.title = meta.name;
    this.mediaInfo.album = meta.album.name;
    this.mediaInfo.durationSeconds = Math.round(
      parseInt(meta.duration.milliseconds) / 1000
    );
    this.mediaInfo.state = Spicetify.Player.data.isPaused
      ? StateMode.PAUSED
      : StateMode.PLAYING;
    switch (Spicetify.Player.data.repeat) {
      case 0: {
        this.mediaInfo.repeatMode = RepeatMode.NONE;
        break;
      }
      case 1: {
        this.mediaInfo.repeatMode = RepeatMode.ALL;
        break;
      }
      case 2: {
        this.mediaInfo.repeatMode = RepeatMode.ONE;
        break;
      }
      default: {
        this.mediaInfo.repeatMode = RepeatMode.NONE;
    ...
    this.mediaInfo.shuffleActive = Spicetify.Player.data.shuffle;
    this.mediaInfo.artist = meta.artists.map((obj) => obj.name).join(", ");

    Spicetify.Platform.LibraryAPI.contains(meta.uri).then(
      ([added]: any) => (this.mediaInfo.rating = added ? 5 : 0)
    );

    const cover = meta.metadata.image_xlarge_url;
    if (cover?.indexOf("localfile") === -1)
      this.mediaInfo.coverUrl =
        "https://i.scdn.co/image/" +
        cover.substring(cover.lastIndexOf(":") + 1);
    else this.mediaInfo.coverUrl = "";
    this.mediaInfo.positionSeconds = Math.round(
      Spicetify.Player.getProgress() / 1000
    );
  ...
  public connectSocket(port: number) {
    this.settings = getSettings();
    let adapter: Adapter | CustomAdapter | undefined = BuiltInAdapters.find(
      (a) => a.port === port
    );
    if (!adapter)
      adapter = this.settings.customAdapters.find((a) => a.port === port);
    if (!adapter) return;

    if (this.sockets.has(port)) return;
    this.sockets.set(
      adapter.port,
      new WNPReduxWebSocket(adapter, this.executeEvent)
    );
  }
...
function ExecuteEventLegacy(self: WNPRedux, message: string) {
  ...
  switch (Events[type as keyof typeof Events]) {
    case Events.PLAYPAUSE: {
      ...
      self.mediaInfo.state =
        self.mediaInfo.state === StateMode.PLAYING
          ? StateMode.PAUSED
          : StateMode.PLAYING;
      break;
    }
    ...
    case Events.SETPOSITION: {
      ...
      const [, positionPercentage] = message
        .toUpperCase()
        .split(":")[1]
        .split("SETPROGRESS ");
    ...
    case Events.REPEAT: {
      ...
      self.mediaInfo.repeatMode =
        self.mediaInfo.repeatMode === RepeatMode.NONE
          ? RepeatMode.ALL
          : self.mediaInfo.repeatMode === RepeatMode.ALL
          ? RepeatMode.ONE
          : RepeatMode.NONE;
      break;
    }
